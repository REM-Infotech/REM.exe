...
class ELAWCrawler:
    ...
    def __init__(self, credentials, input_file):
        ...
        self.input_dir_path = pathlib.Path(input_file).parent.resolve()
    ...
    def login_elaw(self, driver, wait):
        ...
        username.click()
        username.send_keys(self.credentials['login'])
            ...
            for i in range(2, sheet_input.max_row+1):
                ...
                if cell_obj.value is not None and cell_obj.value != '':
                    ...
                    try:
                        ...
                        prt(type='error',message='Erro ao buscar processo {}'.format(numero), row=i-1)
                if i == sheet_input.max_row:
                    prt(type='log',message='Fim dos processos')
        except Exception as e:
            prt(type='error', message='Erro ao carregar processos do input')
